    ...
    void initialize() {
        try {
            ResultSet resultset = Main.statement.executeQuery(
                    "SELECT COUNT(id) FROM marks where marks.mark >=10");
        ...
        b3.setOnAction(actionEvent -> {
            try {
                ResultSet resultset = Main.statement.executeQuery(
                        "SELECT count(id),classOfSt FROM marks WHERE classOfSt=\'"+classs.getText()+"\'AND marks.mark >=10");
                while (resultset.next()){
                    String result = resultset.getString("count(id)");
                    String cl = resultset.getString("classOfSt");
                    textArea.setText("Отличников в "+cl+" классе : "+result);
                }
            } catch (SQLException throwables) {
                throwables.printStackTrace();
            }
